...
\author{单位：武汉大学\quad 姓名：朱威豪\quad 学号：2024302181030}
\date{2026年6月}
...
\reportimage{figures/C1-pipeline-registers.png}{0.94}{流水线 CPU 的 IF/ID、ID/EX、EX/MEM、MEM/WB 四组段寄存器}
...
上述代码展示了流水线 CPU 顶层的模块化连接方式，译码、ALU 和分支判断不再集中写在一个大模块中，而是通过清晰的端口关系接入顶层流水线寄存器推进逻辑。验收时若被要求说明模块划分，可以从该实例化代码解释 ID 阶段如何生成控制信号、EX 阶段如何使用 ALU 结果，以及分支模块如何给出跳转目标。
...
\reportimage{figures/C2-forwarding.png}{0.94}{流水线 CPU 中 forward\_a 与 forward\_b 的 RAW 数据转发判断}
...
\reportimage{figures/C3-load-use-stall.png}{0.94}{流水线 CPU 的 load-use 冒险检测、暂停和 bubble 插入}
...
\reportimage{figures/B2-jalr-branch-flush.png}{0.94}{流水线 CPU 的跳转目标计算、PC 更新和错误路径冲刷}
...
\reportimage{figures/A1-add-decode-idex.png}{0.94}{流水线 add 指令的 R 型译码、IF/ID 保存和 ID/EX 控制信号保存}
...
\reportimage{figures/A2-add-ex-wb.png}{0.94}{流水线 add 指令的转发输入选择、ALU 加法和写回路径}
...
\reportimage{figures/B1-jalr-decode-idex.png}{0.94}{流水线 jalr 指令的译码和 ID/EX 控制信号保存}
...
\reportimage{figures/B2-jalr-branch-flush.png}{0.94}{流水线 jalr 指令的跳转判断、目标地址生成、PC 更新和流水线冲刷}
...
\reportimage{figures/final-all-tests.png}{0.94}{最终扩展指令自动测试 16 个测试点全部通过}

\reportimage{figures/test1-sc-30.png}{0.94}{测试 1：单周期 CPU 运行 30 条指令测试程序}

\reportimage{figures/test2-pl-30.png}{0.94}{测试 2：流水线 CPU 运行 30 条指令测试程序}

\reportimage{figures/test3-sc-sort.png}{0.94}{测试 3：单周期 CPU 运行学号排序程序}
\reportimage{figures/test4-pl-sort.png}{0.94}{测试 4：流水线 CPU 运行学号排序程序}

\section{开发板运行结果}

开发板演示部分采用 Nexys A7 相关封装文件。\texttt{board\_mem.v} 提供上板使用的指令和数据存储接口，\texttt{hex8\_7seg.v} 负责八位七段数码管显示，\texttt{nexys\_a7.xdc} 提供约束文件。演示方式是复位后显示原始学号 \texttt{54873530}，按键触发排序运行后显示 \texttt{03345578}。图中展示了流水线 CPU 上板运行学号排序程序的显示结果，作为测试 5 的硬件运行证据。

\reportimage{figures/test5-board-pl-sort.jpg}{0.55}{测试 5：开发板上流水线 CPU 运行学号排序程序}

\section{代码与 diff 提交说明}

实验代码已上传到个人 GitHub 仓库，并使用差异报告工具将学生实现与教师示例代码进行比较。差异报告分别比较了单周期 CPU 示例目录与 \texttt{source-sc}，以及流水线 CPU 示例目录与 \texttt{source-pipeline}，输出 \texttt{diff.patch} 和 \texttt{report.html}。为了说明代码规模，表 \ref{tab:code-files} 列出本次报告涉及的主要实现文件及当前行数。
...
GitHub 链接：\url{https://github.com/Ikalovic/cpu}

\reportimage{figures/diff-stats.png}{0.94}{Verilog 差异报告顶部统计与修改文件列表}
